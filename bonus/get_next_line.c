...
static char	*file_r(int fd, char *my_static)
{
	...
	if (!my_static)
		my_static = ft_calloc__(1, 1);
	buf = ft_calloc__(BUFFER_SIZE + 1, sizeof(char));
	if (!buf)
		return (NULL);
	read_l = 1;
	while (read_l && !ft_strchr__(my_static, '\n'))
	{
		...
		buf[read_l] = '\0';
		tmp = my_static;
		my_static = ft_strjoin__(tmp, buf);
		free(tmp);
	}
...
static char	*line_g(char **my_static)
{
	...
	if (ft_strlen__(*my_static) && !ft_strchr__ (*my_static, '\n'))
	{
		tmp = ft_substr__(*my_static, 0, ft_strlen__(*my_static));
		return (free_my_static(my_static), tmp);
	}
	else if (ft_strchr__ (*my_static, '\n'))
	{
		i = ft_strlen__(*my_static);
		j = ft_strlen__(ft_strchr__(*my_static, '\n'));
		tmp = ft_substr__(*my_static, 0, i - j + 1);
		str = ft_substr__(ft_strchr__(*my_static, '\n'), 1, j + 1);
